...
[![libraries](../../raw/progress/badge-libraries.svg)](../../tree/progress) [![shaders](../../raw/progress/badge-shaders.svg)](../../tree/progress)

[![progress map](../../raw/progress/progress.svg)](../../tree/progress)
...
[Progress](../../tree/progress), generated by [tools/progress.py](tools/progress.py) on every push to `main`:
- **System libraries**: each block is a [prx library](core/libs/prx) sized by its functions; green is implemented, blue still calls `NotImplemented`. The total only includes functions already declared in the project, not every function of the PS5 firmware.
- **GPU shader instructions**: each block is an instruction encoding of the AMD RDNA 1 + RDNA 2 [instruction list](tools/rdna_isa.txt); green is recognized by the [shader recompiler](core/shader/recompiler).
